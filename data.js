// Employee data for the base64 version of the dynamic employee card.
//
// Each object in the array represents one worker and includes all of the
// fields that appear on the card. To add more employees, append new
// objects to this array. When adding workers you can either:
//  * Provide a `photoData` property containing a data URI (e.g.
//    `data:image/jpeg;base64,...`) representing their photo. This
//    avoids the need for separate image files and is ideal when
//    uploading via GitHub without binary assets.
//  * Or provide a `photo` property containing a URL to the image.
//
// The `ecNo` value is used as an identifier in the URL query
// (e.g. `employee.html?ecNo=JO-I-2025-6000427`).

const employees = [
  {
    ecNo: "JO-I-2025-6000427",
    name: "Lovely Akther",
    ecDate: "30/Sep/2025",
    birthDate: "1995-02-10",
    passportNo: "A17423488",
    passportIssueDate: "2025-01-01",
    passportExpireDate: "2029-12-31",
    visaNo: "800794",
    visaIssueDate: "2025-09-03",
    visaExpireDate: "", // leave blank when not applicable
    referralNo: "",
    recruitingAgency: "BANGLADESH EXPORT CORPORATION (RL0803)",
    employer: "AYYAM COMPANY",
    country: "Jordan",
    bmetNo: "SNF202584269656G",
    gender: "Female",
    bloodGroup: "", // blank means not provided
    nid: "5058567891",
    passportName: "LOVELY AKTHER",
    passportNo1: "A17423488",
    // For the base64 project we embed the employee photo directly as a data URI.
    // This string was generated from the original sample photo and compressed to
    // reduce its size (120x120 JPEG, quality 65).
    photoData: 'data:image/jpeg;base64,',
  },
  // You can add additional employee objects below this line. Each object
  // should have a unique `ecNo` value and the relevant fields filled in.
];

// Expose the employees array globally so it can be accessed by the card
// page. Without this export the `employees` constant would be scoped to
// this file only.
if (typeof window !== 'undefined') {
  window.employees = employees;
}
